...
bool mqtt_start(client clientMQTT, int versionMQTT, int keepalive, int clean_session, int retry) {
  ...
  AT_flag res;
  char buf[300];
  sprintf(buf, "AT+CMQCON=%d,%d,\"%s\",%d,%d,0,\"%s\",\"%s\"", clientMQTT.mqtt_id, versionMQTT, "HzYuHAAnIhsyLQMCASQ4DBo", keepalive, clean_session, "HzYuHAAnIhsyLQMCASQ4DBo", "PCkSmXt32j6XuCjpne8Quecl");
...
bool mqtt_subscribe(client clientMQTT, char *topic, int qos, int retry,  void (*mqttSubcribeCB)(char * data)) {
  AT_flag res;
  char buf[200];
  sprintf(buf, "AT+CMQSUB=%d,\"%s\",%d", clientMQTT.mqtt_id, "channels/2272001/subscribe", qos);
  while(retry--) {
    send_ATComand(buf);
...
bool mqtt_message_publish(client clientMQTT, char *data, char *topic,int qos,  int retry) {
  ...
  sprintf(buf, "AT+CMQPUB=%d,\"%s\",%d,0,0,%d,\"%s\"", clientMQTT.mqtt_id, "channels/2272001/publish", qos, strlen(data)-cnt, data);
  while(retry--)
  {
    send_ATComand(buf);
    res = _readFeedback(3000, "OK");

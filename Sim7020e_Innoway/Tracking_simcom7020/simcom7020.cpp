...
bool mqtt_start(client clientMQTT, int versionMQTT, int keepalive, int clean_session, int retry) {
  ...
  AT_flag res;
  char buf[300];
  sprintf(buf, "AT+CMQCON=%d,%d,\"%s\",%d,%d,0,\"%s\",\"%s\"", clientMQTT.mqtt_id, versionMQTT, clientMQTT.client_id,
   keepalive, clean_session, clientMQTT.user_name, clientMQTT.password); // will flag = 0
  while (retry--) {
    send_ATComand(buf);

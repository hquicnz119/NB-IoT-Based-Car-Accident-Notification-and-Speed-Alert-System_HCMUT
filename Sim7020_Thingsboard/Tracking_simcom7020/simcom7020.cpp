...
bool mqtt_start(client clientMQTT, int versionMQTT, int keepalive, int clean_session, int retry) {
  ...
  AT_flag res;
  char buf[300];
  sprintf(buf, "AT+CMQCON=%d,%d,\"%s\",%d,%d,0,\"%s\",\"%s\"", clientMQTT.mqtt_id, versionMQTT, "bHl28FccGcNty4X6Wh0R", keepalive, clean_session, "bHl28FccGcNty4X6Wh0R", "bHl28FccGcNty4X6Wh0R");
  while (retry--) {
    send_ATComand(buf);
...
bool mqtt_message_publish(client clientMQTT, char *data, char *topic,int qos,  int retry) {
  ...
  sprintf(buf, "AT+CMQPUB=%d,\"%s\",%d,0,0,%d,\"%s\"", clientMQTT.mqtt_id, topic, qos, strlen(data)-cnt, data);
  while(retry--)
  {
    send_ATComand(buf);
    res = _readFeedback(3000, "OK");
